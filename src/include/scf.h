 ...
 * @param src [OUT] 角色来源。
 * @return SCF_SUCCESS、SCF_ERRNO_RBAC_ROLE_UNKNOWN、SCF_ERRNO_CERT_NODE_ID_ABSENT 或错误码。
 * @attention cert 是不透明的裸指针，本接口不会校验证书链、握手状态或证书是否属于 ctx 的对端。
 * 仅可传入 SCF_Connect 或 SCF_Accept 成功后，通过 SCF_GetPeerCert 获取且 ctx 启用了对端校验的证书；
 * 不得传入当前证书、不受信的自签证书或其他调用方可控证书。
 */
